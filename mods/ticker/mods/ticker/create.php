...
if (isset($_POST['submit']))
{
	...
	$cs_ticker['ticker_direction']	= '';
	$cs_ticker['ticker_amount']		= '';
	$cs_ticker['ticker_delay']		= '';
	$cs_ticker['ticker_content']		= '';
}

if(!isset($_POST['submit'])) {
  $data['head']['body'] = $cs_lang['create_text'];
}
elseif(!empty($error)) {
...
if(!empty($error) OR !isset($_POST['submit'])) {
	...
	cs_sql_insert(__FILE__,'ticker',$ticker_cells,$ticker_save);

  // clear datacache
	if (function_exists('cs_datacache_load'))
		cs_datacache_clear(null, 'ticker');

  cs_redirect($cs_lang['create_success'],'ticker');
}
