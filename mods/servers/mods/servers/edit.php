...
// $Id: edit.php 4412 2010-08-20 13:41:26Z Fr33z3m4n $
...
if(isset($_POST['submit'])) {
	...
	$data['create']['servers_rcon'] = $_POST['servers_rcon'];
	...
	$select .= ', servers_type, servers_class, servers_query, servers_order, servers_rcon';
	$where = 'servers_id = \'' . $data['servers']['id'] . '\'';
	$data['create'] = cs_sql_select(__FILE__,'servers',$select,$where,0,0,1);
	$data['create']['servers_rcon'] = cs_encrypt_decrypt($data['create']['servers_rcon']);
}
if(!isset($_POST['submit'])) {
...
if(!empty($servers_error) OR !isset($_POST['submit'])) {
	$games_array = cs_sql_select(__FILE__,'games','games_name, games_id',0,'games_name ASC',0,0);
	$run=0;
	...
	require_once('mods/servers/classes.php');
	$server_stats = array(
