...
// $Id: list.php 1273 2009-03-24 15:35:47Z hajo $
...
$data['pages']['list'] = cs_pages('bets','list',$data['count']['all'],$page_start,$category.'&amp;status='.$bets_status,$sort);

$data['sort']['date'] = cs_sort('bets','list',$page_start,$category,1,$sort,'status='.$bets_status);
$data['sort']['title'] = cs_sort('bets','list',$page_start,$category,3,$sort,'status='.$bets_status);
$data['sort']['category'] = cs_sort('bets','list',$page_start,$category,5,$sort,'status='.$bets_status);
...
// DATENBANKABFRAGE
$where = 'bets_status =' . $bets_status;
$where .= $category != 0 ?  ' AND bet.categories_id = ' . $category . ' GROUP BY bet.bets_id' : ' GROUP BY bet.bets_id';
...
$select .= ', bet.bets_starts_at AS bets_starts_at, COUNT(busr.bets_id) AS anzahl, bet.categories_id AS cat_id';
$from = 'bets bet LEFT JOIN {pre}_categories cat ON cat.categories_id = bet.categories_id LEFT JOIN {pre}_bets_users busr ON busr.bets_id = bet.bets_id';
...
echo cs_subtemplate(__FILE__,$data,'bets','list');
?>
