...
if (isset($_POST['submit']))
{
  $error = 0;
	...
	$teamspeak_id = intval($_POST['teamspeak_id']);
  ...
  $cells = 'teamspeak_id, teamspeak_access, teamspeak_version, teamspeak_ip, teamspeak_udp, teamspeak_tcp, teamspeak_admin, teamspeak_adminpw, teamspeak_sadmin, teamspeak_sadminpw, teamspeak_register, teamspeak_charset';
  $cs_teamspeak = cs_sql_select(__FILE__,'teamspeak',$cells,'teamspeak_id = ' . $teamspeak_id);
  $cs_teamspeak['teamspeak_adminpw'] = cs_crypt(base64_decode($cs_teamspeak['teamspeak_adminpw']), $cs_main['crypt_key']);
  $cs_teamspeak['teamspeak_sadminpw'] = cs_crypt(base64_decode($cs_teamspeak['teamspeak_sadminpw']), $cs_main['crypt_key']);
}
...
if(!empty($error) OR !isset($_POST['submit'])) {
	$data['teamspeak']['teamspeak_id'] = $teamspeak_id;
	...
	echo cs_subtemplate(__FILE__,$data,'teamspeak','serveredit');
}
else {
  ...
  cs_sql_update(__FILE__,'teamspeak',$teamspeak_cells,$teamspeak_save,$teamspeak_id);
  
  cs_redirect($cs_lang['changes_done'],'teamspeak');
} 
?>
