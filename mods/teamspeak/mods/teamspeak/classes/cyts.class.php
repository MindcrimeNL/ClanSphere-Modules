...
/**
  ...
  */
	function connect($sIP, $sTCP, $sUDP = false, $sTimeout = 3) {
		...
		/* Now that we are connected, set a timeout on the stream for reading/writing.
		 * By default set it to 2 times the connection timeout
		 */
		stream_set_timeout($this->sCon, $sTimeout * 2);
...
/**
  ...
  */  	
	function _fastcall($sCall = "") {
		...
		if (!is_array($sCall))
		{
			$writeResult = fwrite($this->sCon, "$sCall\n");
			if ($writeResult === false || $writeResult < strlen($sCall)+1)
				return false;
			...
			foreach ($sCall as $sCmd) {
				$writeResult = fwrite($this->sCon, "$sCmd\n");
				if ($writeResult === false || $writeResult < strlen($sCmd)+1)
					return false;
				$cRes = $this->_readcall();
