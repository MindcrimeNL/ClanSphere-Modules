...
/**
  ...
  * @return     boolean success
  */
	public function selectServerByPort($port) {
...
/**
  ...
  */
	public function clientDbList($params = "") {
		if(!$this->selected) { 
...
/**
  ...
  * @return     boolean success
  */
	public function clientMove($clid, $cid) {
		if(!$this->selected) { 
...
/**
  ...
  * @return     boolean success
  */
	public function clientKick($clid, $from, $kickmsg = "") {
		if(!$this->selected) { 
...
/**
  ...
  * @return     boolean success
  */
	public function clientBan($clid, $time, $banreason = "") {
		if(!$this->selected) { 
...
/**
  ...
  * @return     multidimensional array clientlist 
  */
	public function clientFind($pattern, $params = "") {
		if(!$this->selected) { 
...
/**
  ...
  * @return     boolean success
  */
	public function clientEdit($clid, $newSettings) {
		if(!$this->selected) { 
...
/**
  ...
  * @return     multidimensional array clientlist 
  */
	public function clientDbFind($pattern, $params = "") {
		if(!$this->selected) { 
...
/**
  ...
  * @return     boolean success
  */
	public function clientDbEdit($cldbid, $newSettings) {
		if(!$this->selected) { 
...
/**
  ...
  * @return     multidimensional array clientlist 
  */
	public function clientGetIds($cluid, $params = "") {
		if(!$this->selected) { 
...
/**
  ...
  */
	public function serverList($params = "") {
		if(!empty($params)) { $params = ' '.$params; }
...
/**
  ...
  */
	public function getServerIdByPort($port) {
...
/**
  ...
  */
	public function complainList($params = "") {
		if(!$this->selected) { 
...
/**
  * channelGroupList: displays a list of channel groups available<br>
  ...
  */
	public function channelGroupList($params = "") {
		if(!$this->selected) { 
...
/**
  ...
  */
	public function serverGroupList($params = "") {
		if(!$this->selected) { 
...
/**
  ...
  * @return     boolean success
  */
	public function serverGroupAdd($name) {
		if(!$this->selected) { 
...
/**
  ...
  * @return     boolean success
  */
	public function serverGroupDelete($sgid, $force) {
		if(!$this->selected) { 
...
/**
  ...
  * @return     boolean success
  */
	public function serverGroupPermList($sgid, $params = "") {
		if(!$this->selected) { 
...
/**
  ...
  * @return     boolean success
  */
	public function serverGroupAddPerm($sgid, $permid, $permvalue, $permnegated, $permskip) {
		if(!$this->selected) { 
...
/**
  ...
  * @return     boolean success
  */
	public function serverGroupDeletePerm($sgid, $permid) {
		if(!$this->selected) { 
...
/**
  ...
  */
	public function serverGroupClientList($sgid, $params = "") {
		if(!$this->selected) { 
...
/**
  ...
  */
	public function serverGroupsByClientID($sgid) {
		if(!$this->selected) { 
...
/**
  * tokenAdd: Adds a token for a specific type
  *		Note: login needed
  *
  ...
  *
  * @param	int	$type	0 (server) or 1 (channel)
  * @param	int	$id1	server group id or channel group id
  * @param	int	$id2	channel id (if type is 1)
  */
	public function tokenAdd($type, $id1, $id2)
	{
		if(!$this->selected) { 
			$this->debug .= "Error in tokenAdd(): you have to select a server befor using tokenAdd()<br>"; 
			return false;
		}
		return $this->getSimpleData("tokenadd tokentype=$type tokenid1=$id1 tokenid2=$id2");
	}

 	/**
   * tokenAddCustomSet: Adds a token for a specific type with custom fields
   *		Note: login needed
   *
   * Create a new token. If tokentype is set to 0, the ID specified with tokenid1
   * will be a server group ID. Otherwise, tokenid1 is used as a channel group ID
   ...
   * @param	string $description
	 ...
	 * @since	3.0.0-beta15
   */
	public function tokenAddCustomSet($type, $id1, $id2, $customfields, $description='Custom Token')
	{
		if(!$this->selected) { 
			$this->debug .= "Error in tokenAddCustomSet(): you have to select a server befor using tokenAddCustomSet()<br>"; 
			return false;
		}
		$customset = '';
		foreach ($customfields as $key => $value)
		{
			if (!empty($customset))
				$customset .= '|';
			$customset .= 'ident='.$key.' value='.$value; 
		}
		$customset = $this->implodeText($customset);
		$description = $this->implodeText($description);
		return $this->getSimpleData("tokenadd tokentype=$type tokenid1=$id1 tokenid2=$id2 tokendescription=$description tokencustomset=$customset");
	} // function tokenAddCustomSet
	
	/**
	 * Search for a client based on a specific ident / pattern pair
	 * 
	 * @param	string	$ident	the custom field
	 * @param	string	$pattern	the (including SQL) pattern to search
	 *
	 * @since	3.0.0-beta15
	 */
	...
	{
		if(!$this->selected) { 
			$this->debug .= "Error in customSearch(): you have to select a server befor using customSearch()<br>"; 
			return false;
		}
		$ident  = $this->implodeText($ident);
		$pattern  = $this->implodeText($pattern);
		return $this->getExtendData("customsearch ident=$ident pattern=$pattern");
 ...
 * @access	private
*/
	public function __construct($host = '127.0.0.1', $queryport = '10011', $timeout = 2) {
		$this->host = $host;
...
/**
  ...
  * @return		boolean success
  */
	public function connect($host = null, $queryport = null, $timeout = null) {
		if (!is_null($host)) $this->host = $host;
		if (!is_null($queryport)) $this->queryport = $queryport;
		if (!is_null($timeout)) $this->timeout = $timeout;
		if (is_resource($this->socket)) { $this->debug .= 'Error: You are already connected!<br>'; return false; }
		...
		if(!$socket) {
			...
			if(strpos(fgets($this->socket), 'TS3') !== false) {
				/* Since BETA 29, try to get extra welcome message */
				if (strpos(fgets($this->socket), 'Welcome') !== false)
				  $this->debug .= 'Welcome message (BETA 29+ Server) received<br>';
				return true;
			}else{
				return false;
			}
...
/**
  ...
  */
	private function executeCommand($command) {
		if (!is_resource($this->socket)) { $this->debug .= 'Error: no connection found, you have to use connect() method!<br>'; return false; }
		$data = '';
