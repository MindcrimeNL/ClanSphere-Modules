...
$select .= 'thr.users_id AS users_id, thr.threads_text AS threads_text, frm.squads_id AS squads_id';
...
if(isset($_POST['agree'])) {
  ...
  cs_sql_delete(__FILE__,'threads',$thread_id);
	include('mods/coins/thread_remove.php');
...
if(isset($_POST['cancel']))
 cs_redirect($cs_lang['del_false'],'board','thread','where=' .$thread_id);
	...
	$data['thread']['coins_checked'] = empty($cs_option['coins_checkbox_thread']) ? '' : ' checked="checked"';
	$lang_coins = cs_translate('coins');
	$data['thread']['lang_coins_rollback'] = $lang_coins['coins_rollback_thread'];
