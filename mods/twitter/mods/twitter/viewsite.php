...
$key = 'lang='.$account['users_lang'].'&start='.$start.'&size='.$account['users_limit'].'&access='.$account['access_twitter'];
$cachedata = cs_datacache_load('twitter', 'viewsite', $key, false);
if ($cachedata !== false)
{
...
try
{
	...
	$cachedata = cs_subtemplate(__FILE__,$data,'twitter','viewsite');
	cs_datacache_create('twitter', 'viewsite', $key, $cachedata, 60);
}
catch (Exception $e)
{
