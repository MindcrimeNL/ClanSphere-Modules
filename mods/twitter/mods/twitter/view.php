...
$cachedata = cs_datacache_load('twitter', 'view', $key, false);
if ($cachedata !== false)
{
...
try
{
	...
	$data = array();
	$twitters = $twitter->statusesFriendsTimeline(null, null, $account['users_limit'], floor($start / $account['users_limit']) + 1);
	$run = 0;
	...
	$cachedata = cs_subtemplate(__FILE__,$data,'twitter','view');
	cs_datacache_create('twitter', 'view', $key, $cachedata, 60);
}
catch (Exception $e)
{
