...
<table class="forum" style="width:{page:width}" cellpadding="0" cellspacing="{page:cellspacing}">
	...
	<tr>
		<td class="leftc" valign="top">{icon:fileshare} {lang:plugins}:</td>
		<td class="leftb"><input type="text" name="plugins" value="{op:plugins}" maxlength="80" size="50" /><br />
				{if:plugin}
				<table>
				{loop:plugins}
				<tr>
					<td class="leftb" valign="top" colspan="2"><b>{plugins:fullname}:</b></td>
				</tr>
				<tr>
					<td class="leftb" valign="top">{lang:game}:</td>
					<td class="leftb"><select name="games_ids_{plugins:name}[]" size="5" multiple="true">{plugins:games_options}</select></td>
				</tr>
				{plugins:extra_options}
				{stop:plugins}
				</table>
				{stop:plugin}
			</td>
