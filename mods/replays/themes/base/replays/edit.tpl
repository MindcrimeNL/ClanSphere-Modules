...
<table class="forum" style="width:{page:width}" cellpadding="0" cellspacing="{page:cellspacing}">
	...
	<tr>
		...
		<td class="leftb">
			<select name="games_id" onchange="{if:plugin}replays_plugins_enable(this.form.games_id.options[this.form.games_id.selectedIndex].value);{stop:plugin}document.getElementById('game_1').src='{page:path}uploads/games/' + this.form.games_id.options[this.form.games_id.selectedIndex].value + '.gif'">
	...
	{if:plugin}
	<tr>
		<td class="leftc" valign="top">{icon:package_games} {lang:plugins}</td>
		<td class="leftb">
				<script type="text/javascript">
				function replays_plugins_enable(games_id)
				{
{plugin:javascript}
				}
				</script>
				{loop:plugins}
				<input type="checkbox" name="plugin_{plugins:name}" id="plugin_{plugins:name}" value="1"{plugins:disabled}{plugins:checked}> {plugins:fullname}<br />{plugins:info}
				{plugins:extra}<br />
				{stop:plugins}
		</td>
	</tr>
