...
// $Id: view.php 4120 2010-06-15 15:29:39Z hajo $
...
$select .= 'replays_team2, replays_date, replays_map, replays_mirror_urls, replays_mirror_names, replays_info, replays_id, replays_close, replays_access, replays_plugins';
...
if(!empty($cs_replays['replays_mirror_urls'])) {
  ...
  for($run=0; $run < $mirror_count; $run++) {
    if (empty($mirror_names[$run])) $mirror_names[$run] = $mirror_urls[$run];
  	...
  	if ($run == 0)
  	{
  		$rpath = realpath($mirror_urls[$run]);
  		if (!empty($rpath))
				$useDownload = true;
  	}

  	if ($useDownload)
  		$data['replays']['mirrors'] .= cs_html_link('mods/replays/download.php?id='.$cs_replays_id, $mirror_urls[$run]) . cs_html_br(1);
  	else
...
/* do plugin stuff */
$selplugins = empty($cs_replays['replays_plugins']) ? array() : explode(',', $cs_replays['replays_plugins']);
...
if (count($selplugins))
{
	...
	foreach ($plugins as $pname => $pinfo) 
	{
		if (in_array($pname, $selplugins))
		{
			...
			$data['plugins'][$run]['view'] = call_user_func_array('replays_plugins_view_'.$pname, array($pinfo, $cs_replays_id));
			$run++;
		}
	}
}
