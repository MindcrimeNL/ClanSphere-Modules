...
if(isset($_POST['submit'])) {
  ...
  $cs_replays['games_id'] = intval($_POST['games_id']);
  ...
  while($levels < 6) {
	...
	if (count($plugins))
	{
		...
		foreach ($plugins as $pname => $pinfo) 
		{
			...
			$data['plugins'][$run]['checked'] = !empty($_POST['plugin_'.$pname]) ? ' checked="checked"' : ''; 
  ...
  if(!empty($files_gl['replay']['tmp_name'])) {
    ...
    cs_sql_update(__FILE__,'replays',$replays2_cells,$replays2_save,$getid['replays_id']);
	  if (!empty($cs_replays['replays_plugins']))
	  {
	  	...
	  	replays_plugins_create($plugins, $execplugins, $getid['replays_id']);
	  }
  }
