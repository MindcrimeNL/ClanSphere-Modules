...
define('WC3_REPLAY_MAX_DATABLOCK', 1500);
// for preventing duplicated actions
define('WC3_REPLAY_ACTION_DELAY', 1000);
// to know how long it may take after buying a Tome of Retraining to retrain a hero
define('WC3_REPLAY_RETRAINING_TIME', 15000);

class replay_wc3 {
	public $fp, $data, $leave_unknown, $continue_game, $referees, $time, $pause, $leaves;
	public $errors, $header, $game,  $players, $teams, $chat, $filename, $parse_actions, $parse_chat;
	public $max_datablock = WC3_REPLAY_MAX_DATABLOCK;
	public $apmdata = array('new_segment' => array(),
		'lastactions' => array(),
		'lasttime' => array(),
		'sumactions' => array(),
		'sumtime' => array(),
	...
	protected function _changeElemList($elemlist, $new_element)
	{
	...
	protected function _parsedata() {
		...
		for ($i=0; $i<$blocks_count; $i++) {
			...
			if ($temp = gzinflate($temp)) {
				...
				$this->setError($this->filename.': Incomplete replay file');
				return false;
			}
	...
	protected function _loadplayer() {
		...
		$this->players[$player_id]['runits_time'] = 0; // prevent PHP warnings
		$this->players[$player_id]['units_time'] = 0; // prevent PHP warnings
		$this->players[$player_id]['rupgrades_time'] = 0; // prevent PHP warnings
		$this->players[$player_id]['upgrades_time'] = 0; // prevent PHP warnings
		$this->players[$player_id]['actions_details'] = array(); // prevent PHP warnings
		$this->players[$player_id]['hotkeys'] = array(); // prevent PHP warnings
		$this->players[$player_id]['last_itemid'] = 0; // prevent PHP warnings
		$this->players[$player_id]['player_id'] = $player_id;
		$this->players[$player_id]['apmcount'] = array('maxmedianapm' => 0); // prevent PHP warnings

		$this->players[$player_id]['initiator'] = w3g_convert_bool(!$temp['record_id']);

		$this->apmdata['new_segment'][$player_id] = 0;
		$this->apmdata['lastactions'][$player_id] = array(0 => 0, 'allactions' => 0);
		$this->apmdata['lasttime'][$player_id] = array(0 => 0, 'lastsumtime' => 0);
		$this->apmdata['sumactions'][$player_id] = 0;
		$this->apmdata['sumtime'][$player_id] = 0;
		...
		if (ord($this->data{0}) == 1) { // custom game
			...
			$this->players[$player_id]['race'] = w3g_convert_race($temp['race']);
		}
	...
	protected function _loadgame() {
		...
		// 4.4 [GameSettings]
		$this->game['speed'] = w3g_convert_speed(ord($temp{0}));
		
		if (ord($temp{1}) & 1) {
			$this->game['visibility'] = w3g_convert_visibility(0);
		} else if (ord($temp{1}) & 2) {
			$this->game['visibility'] = w3g_convert_visibility(1);
		} else if (ord($temp{1}) & 4) {
			$this->game['visibility'] = w3g_convert_visibility(2);
		} else if (ord($temp{1}) & 8) {
			$this->game['visibility'] = w3g_convert_visibility(3);
		}
		$this->game['observers'] = w3g_convert_observers(((ord($temp{1}) & 16) == true) + 2*((ord($temp{1}) & 32) == true));
		$this->game['teams_together'] = w3g_convert_bool(ord($temp{1}) & 64);
		
		$this->game['lock_teams'] = w3g_convert_bool(ord($temp{2}));
		
		$this->game['full_shared_unit_control'] = w3g_convert_bool(ord($temp{3}) & 1);
		$this->game['random_hero'] = w3g_convert_bool(ord($temp{3}) & 2);
		$this->game['random_races'] = w3g_convert_bool(ord($temp{3}) & 4);
		if (ord($temp{3}) & 64) {
			$this->game['observers'] = w3g_convert_observers(4);
		}
		...
		// 4.7 [GameType]
		$this->game['type'] = w3g_convert_game_type(ord($this->data[0]));
		$this->game['private'] = w3g_convert_bool(ord($this->data[1]));
		...
		for ($i=0; $i<$slot_records; $i++) {
			...
			if ($temp['slot_status'] == 2) { // do not add empty slots
				$temp['color_html'] = w3g_convert_color_html($temp['color']);
				$temp['color'] = w3g_convert_color($temp['color']);
				$temp['race'] = w3g_convert_race($temp['race']);
				$temp['ai_strength'] = w3g_convert_ai($temp['ai_strength']);
		...
		$this->game['select_mode'] = w3g_convert_select_mode($temp['select_mode']);
			...
			switch ($block_id) {
				...
				case 0x20:
					...
					if ($this->header['major_v'] > 2) {
						...
						if ($temp['flags'] == 0x20) {
							$temp['mode'] = w3g_convert_chat_mode($temp['mode']);
				...
				case 0:
					...
					break;
				default:
					if (isset($this->errors[$this->time]))
						$this->errors[$this->time] .= '. Unhandled replay command block at '.w3g_convert_time($this->time).': 0x'.sprintf('%02X', $block_id).' (prev: 0x'.sprintf('%02X', $prev).', time: '.$this->time.') in '.$this->filename;
					else
						$this->errors[$this->time] = 'Unhandled replay command block at '.w3g_convert_time($this->time).': 0x'.sprintf('%02X', $block_id).' (prev: 0x'.sprintf('%02X', $prev).', time: '.$this->time.') in '.$this->filename;
					return false;
			}
	...
	protected function _parseactions($actionblock, $data_length) {
		...
		while ($data_length) {
			...
			while ($n < $block_length) {
				...
				switch ($action) {
					...
					case 0x10:
						...
						$value = w3g_convert_itemid($itemid);
						
						if (!$value) {
							$this->players[$player_id]['actions_details'][w3g_convert_action('ability')]++;
							
							// handling Destroyers
							if (ord($actionblock{$n+2}) == 0x33 && ord($actionblock{$n+3}) == 0x02) {
								$name = substr(w3g_convert_itemid('ubsp'), 2);
								$this->players[$player_id]['units']['order'][$this->time] = $this->players[$player_id]['units_multiplier'].' '.$name;
								if (!isset($this->players[$player_id]['units'][$name]))
									$this->players[$player_id]['units'][$name] = 0;
								$this->players[$player_id]['units'][$name]++;
								
								$name = substr(w3g_convert_itemid('uobs'), 2);
								$this->players[$player_id]['units'][$name]--;
							}
						} else {
							if (!isset($this->players[$player_id]['actions_details'][w3g_convert_action('buildtrain')]))
								$this->players[$player_id]['actions_details'][w3g_convert_action('buildtrain')] = 0;
							$this->players[$player_id]['actions_details'][w3g_convert_action('buildtrain')]++;
							
							if (!isset($this->players[$player_id]['race_detected']) || !$this->players[$player_id]['race_detected']) {
								if ($race_detected = w3g_convert_race($itemid)) {
							...
							switch ($value{0}) {
								case 'u':
									...
									if (($this->time - $this->players[$player_id]['units_time'] > WC3_REPLAY_ACTION_DELAY || $itemid != $this->players[$player_id]['last_itemid'])
								...
								case 'h':
									...
									if (!isset($this->players[$player_id]['heroes'][$name]))
										$this->players[$player_id]['heroes'][$name] = array('revivals' => 0);
								...
								case 'a':
									...
									if (!isset($this->players[$player_id]['heroes'][$hero]['retraining_time']) || !$this->players[$player_id]['heroes'][$hero]['retraining_time']) {
										$this->players[$player_id]['heroes'][$hero]['retraining_time'] = 0;
									}
									
									// prevent PHP warnings
									if (!isset($this->players[$player_id]['heroes'][$hero]['level']))
										$this->players[$player_id]['heroes'][$hero]['level'] = 0;
									if (!isset($this->players[$player_id]['heroes'][$hero]['ability_time']))
										$this->players[$player_id]['heroes'][$hero]['ability_time'] = 0;
									if (!isset($this->players[$player_id]['heroes'][$hero]['abilities']))
										$this->players[$player_id]['heroes'][$hero]['abilities'] = array($retraining_time => array($ability => 0));
									if (!isset($this->players[$player_id]['heroes'][$hero]['abilities'][$retraining_time][$ability]))
									...
									if (($this->time - $this->players[$player_id]['heroes'][$hero]['ability_time'] > WC3_REPLAY_ACTION_DELAY
									|| !$this->players[$player_id]['heroes'][$hero]['ability_time']
									|| $this->time - $retraining_time < WC3_REPLAY_RETRAINING_TIME)
									&& $this->players[$player_id]['heroes'][$hero]['abilities'][$retraining_time][$ability] < 3) {
										if ($this->time - $retraining_time > WC3_REPLAY_RETRAINING_TIME) {
								...
								case 'i':
									...
									if (!isset($this->players[$player_id]['items'][$name]))
								...
								case 'p':
									...
									if ($this->time - $this->players[$player_id]['upgrades_time'] > WC3_REPLAY_ACTION_DELAY || $itemid != $this->players[$player_id]['last_itemid']) {
								...
								default:
									if (isset($this->errors[$this->time]))
										$this->errors[$this->time] .= '. Unknown ItemID at '.w3g_convert_time($this->time).': '.$value;
									else
										$this->errors[$this->time] = 'Unknown ItemID at '.w3g_convert_time($this->time).': '.$value;
									break;
							}
					...
					case 0x11:
						...
						if (ord($actionblock{$n+2}) <= 0x19 && ord($actionblock{$n+3}) == 0x00) { // basic commands
							if (!isset($this->players[$player_id]['actions_details'][w3g_convert_action('basic')]))
								$this->players[$player_id]['actions_details'][w3g_convert_action('basic')] = 0;
							$this->players[$player_id]['actions_details'][w3g_convert_action('basic')]++;
						} else {
							if (!isset($this->players[$player_id]['actions_details'][w3g_convert_action('ability')]))
								$this->players[$player_id]['actions_details'][w3g_convert_action('ability')] = 0;
							$this->players[$player_id]['actions_details'][w3g_convert_action('ability')]++;
						}
						$value = strrev(substr($actionblock, $n+2, 4));
						if ($value = w3g_convert_buildingid($value)) {
					...
					case 0x12:
						...
						if (ord($actionblock{$n+2}) == 0x03 && ord($actionblock{$n+3}) == 0x00) { // rightclick
							if (!isset($this->players[$player_id]['actions_details'][w3g_convert_action('rightclick')]))
								$this->players[$player_id]['actions_details'][w3g_convert_action('rightclick')] = 0;
							$this->players[$player_id]['actions_details'][w3g_convert_action('rightclick')]++;
					...
					case 0x13:
						...
						if (!isset($this->players[$player_id]['actions_details'][w3g_convert_action('item')]))
							$this->players[$player_id]['actions_details'][w3g_convert_action('item')] = 0;
						$this->players[$player_id]['actions_details'][w3g_convert_action('item')]++;
						if ($this->header['major_v'] >= 7) {
							$n+=38;
						} else {
							$n+=30;
					...
					case 0x16:
						...
						if ($temp['mode'] == 0x02 || !$was_deselect) {
							$this->players[$player_id]['actions']++;
							if (!isset($this->players[$player_id]['actions_details'][w3g_convert_action('select')]))
								$this->players[$player_id]['actions_details'][w3g_convert_action('select')] = 0;
							$this->players[$player_id]['actions_details'][w3g_convert_action('select')]++;
						}
					...
					case 0x17:
						$this->players[$player_id]['actions']++;
						if (!isset($this->players[$player_id]['actions_details'][w3g_convert_action('assignhotkey')]))
							$this->players[$player_id]['actions_details'][w3g_convert_action('assignhotkey')] = 0;
						$this->players[$player_id]['actions_details'][w3g_convert_action('assignhotkey')]++;
						$temp = unpack('Cgroup/vnum', substr($actionblock, $n+1, 3));
						if (!isset($this->players[$player_id]['hotkeys'][$temp['group']]))
							$this->players[$player_id]['hotkeys'][$temp['group']] = array('assigned' => 0, 'last_totalitems' => 0);
					...
					case 0x18:
						$this->players[$player_id]['actions']++;
						if (!isset($this->players[$player_id]['actions_details'][w3g_convert_action('selecthotkey')]))
							$this->players[$player_id]['actions_details'][w3g_convert_action('selecthotkey')] = 0;
						$this->players[$player_id]['actions_details'][w3g_convert_action('selecthotkey')]++;
						if (!isset($this->players[$player_id]['hotkeys'][ord($actionblock{$n+1})]['used']))
							$this->players[$player_id]['hotkeys'][ord($actionblock{$n+1})]['used'] = 0;
					...
					case 0x19:
						...
						if ($this->header['build_v'] >= 6040 || $this->header['major_v'] > 14) {
							if ($was_subgroup) { // can't think of anything better (check action 0x1A)
								$this->players[$player_id]['actions']++;
								if (!isset($this->players[$player_id]['actions_details'][w3g_convert_action('subgroup')]))
									$this->players[$player_id]['actions_details'][w3g_convert_action('subgroup')] = 0;
								$this->players[$player_id]['actions_details'][w3g_convert_action('subgroup')]++;
					...
					case 0x1E:
						...
						if (($this->header['build_v'] >= 6040 || $this->header['major_v'] > 14) && $action != 0x1E) {
							...
							$this->players[$player_id]['actions']++;
							if (!isset($this->players[$player_id]['actions_details'][w3g_convert_action('removeunit')]))
								$this->players[$player_id]['actions_details'][w3g_convert_action('removeunit')] = 0;
							$this->players[$player_id]['actions_details'][w3g_convert_action('removeunit')]++;
							$value = w3g_convert_itemid(strrev(substr($actionblock, $n+2, 4)));
							$name = substr($value, 2);
							switch ($value{0}) {
								case 'u':
									// preventing duplicated units cancellations
									if ($this->time - $this->players[$player_id]['runits_time'] > WC3_REPLAY_ACTION_DELAY || $value != $this->players[$player_id]['runits_value']) {
								...
								case 'p':
									...
									if ($this->time - $this->players[$player_id]['rupgrades_time'] > WC3_REPLAY_ACTION_DELAY || $value != $this->players[$player_id]['rupgrades_value']) {
					...
					case 0x61:
						$this->players[$player_id]['actions']++;
						if (!isset($this->players[$player_id]['actions_details'][w3g_convert_action('esc')]))
							$this->players[$player_id]['actions_details'][w3g_convert_action('esc')] = 0;
						$this->players[$player_id]['actions_details'][w3g_convert_action('esc')]++;
						++$n;
						break;
					...
					case 0x65:
						$this->players[$player_id]['actions']++;
						if (!isset($this->players[$player_id]['actions_details'][w3g_convert_action('heromenu')]))
							$this->players[$player_id]['actions_details'][w3g_convert_action('heromenu')] = 0;
						$this->players[$player_id]['actions_details'][w3g_convert_action('heromenu')]++;
						++$n;
						break;
					...
					case 0x66:
						...
						if ($this->header['major_v'] >= 7) {
							...
							if (!isset($this->players[$player_id]['actions_details'][w3g_convert_action('buildmenu')]))
								$this->players[$player_id]['actions_details'][w3g_convert_action('buildmenu')] = 0;
							$this->players[$player_id]['actions_details'][w3g_convert_action('buildmenu')]++;
						}
						$n+=1;
						break;
					...
					case 0x01:
						...
						$temp = '';
						$temp['time'] = $this->time;
						$temp['text'] = w3g_convert_chat_mode(0xFE, $this->players[$player_id]['name']);
						$this->chat[] = $temp;
						$n+=1;
						break;
					...
					case 0x02:
						...
						$this->pause = false;
						$temp['time'] = $this->time;
						$temp['text'] = w3g_convert_chat_mode(0xFF, $this->players[$player_id]['name']);
						$this->chat[] = $temp;
						$n+=1;
						break;
					...
					default:
						...
						if (isset($this->errors[$this->time]))
							$this->errors[$this->time] .= '. Unknown action at '.w3g_convert_time($this->time).': 0x'.sprintf('%02X', $action).', prev: 0x'.sprintf('%02X', $prev).', dump: '.$temp;
						else
							$this->errors[$this->time] = 'Unknown action at '.w3g_convert_time($this->time).': 0x'.sprintf('%02X', $action).', prev: 0x'.sprintf('%02X', $prev).', dump: '.$temp;
			...
			// Function _changeElemList at beginning of parser
			if($this->apmdata['new_segment'][$player_id] < ($this->time))
			{
				...
				if(isset($this->players[$player_id]))
				{
					if($this->players[$player_id]['actions'] - $this->apmdata['lastactions'][$player_id]['allactions'] > 0)
					{
						...
						$this->apmdata['lastactions'][$player_id] = $this->_changeElemList($this->apmdata['lastactions'][$player_id], $newelement);
						// time - sequence
						...
						$this->apmdata['lasttime'][$player_id] = $this->_changeElemList($this->apmdata['lasttime'][$player_id], $newelement);
						// median
	...
	protected function _cleanup() {
		...
		if (isset($this->game['winner_team']) && $this->game['winner_team'] == 99) { // saver
			$this->game['winner_team'] = $this->players[$this->game['saver_id']]['team'];
		} elseif (isset($this->game['loser_team']) && $this->game['loser_team'] == 99) {
			$this->game['loser_team'] = $this->players[$this->game['saver_id']]['team'];
		}
	
		$winner = isset($this->game['winner_team']) ? strlen($this->game['winner_team']) : 0;
		$loser = isset($this->game['loser_team']) ? strlen($this->game['loser_team']) : 0;
		if (!$winner && $loser) {
