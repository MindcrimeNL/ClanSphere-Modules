...
$cs_cup = cs_sql_select(__FILE__,'cups','cups_start, cups_system, cups_teams, cups_access','cups_id = ' . $cups_id);
$started = cs_sql_count(__FILE__,'cupmatches','cups_id = ' . $cups_id);
$joined = cs_sql_count(__FILE__,'cupsquads','cupsquads_checkedin = 1 AND cups_id = ' . $cups_id);

$form = 1;
$full = $joined >= $cs_cup['cups_teams'] ? 1 : 0;

if ($cs_cup['cups_system'] == CS_CUPS_TYPE_TEAMS && empty($full)) {
...
if ($account['access_cups'] >= 2 && $cs_cup['cups_access'] != 0 && $cs_cup['cups_access'] <= $account['access_cups'] && cs_time() < $cs_cup['cups_start'] && empty($started) && empty($full)) {
  ...
  elseif(empty($_POST['submit'])) {
    ...
    if ($cs_cup['cups_system'] == CS_CUPS_TYPE_TEAMS) {
    ...
    if ($_POST['system'] == CS_CUPS_TYPE_TEAMS) {
